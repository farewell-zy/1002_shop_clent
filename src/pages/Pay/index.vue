...
export default {
  ...
  methods: {
    ...
    //3、轮询
    async pay() {
      // 第一步,根据支付信息当中的codeUrl生成二维码进行弹窗展示
      ...
      try {
        ...
        this.$alert(
          ...
          '请使用微信扫码支付', 
          {
            ...
            center: true,
            // beforeClose一旦设定，那么消息盒子关闭不关闭，我们可以在这个里面控制
            beforeClose:(action, instance, done) => {
              // action代表用户点击的是哪个按钮 
              // 'confirm'确定按钮 'cancel'取消按钮 'close'关闭
              if(action === 'confirm'){
                // 代表用户点击的是确定
                // 如果没有支付
                if(!this.payStatus){
                ...
                // 2、清除定时器
                clearInterval(this.timer) //clearInterval只是清除了定时器，不让管理模块中的定时器起作用
                this.timer = null // 把之前设置定时器的标号id也清除
                // 3、关闭消息盒子
                done() // 调用就会关闭，不调就不关
              }
            }
            ...
            .catch(() => {})
            // .then()是对应点击确定按钮之后的操作
            // 如果采用这两个操作，都会强制关闭弹窗msgBox
            // .catch()是对应点击取消按钮之后的操作
          
          

        // 轮询 隔2s发一个请求，为了让后台给我返回这个支付状态
        // 用以判断用户是否支付
        if(!this.timer){ // 这里必须添加判断，确保这个订单支付轮询只开启一个定时器
          this.timer = setInterval(async ()=>{
            const result = await this.$API.reqPayStatus(this.orderNum);
            if(result.code===200){
              // 代表支付状态成功
              // 1、把成功的标志存储起来，用于用户点击按钮的时候进行判断
              this.payStatus = 200
              // 2、提示支付成功
