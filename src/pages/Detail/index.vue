          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                -->
                <input autocomplete="off" class="itxt" v-model="skuNum" :change="$event.target.value >= 1?skuNum=parseInt($event.target.value*1):skuNum=1"/>
...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValueList,spuSaleAttrValue){
      // 排他所有的改为一种状态
      spuSaleAttrValueList.forEach(item => item.isChicked = '0');
      // 点击的改为另一种状态
      spuSaleAttrValue.isChicked = '1'
    },
    // 点击添加购物车逻辑
    async addShopCart(){
      // 第一步：先发请求添加购物车
      // 我们只能通过dispatch的返回值，来判断请求成功还是失败
      // 第一种，能用但不合理
      ...
      //   // 代表请求失败
      // }
      // 第二种 action的异步函数，返回的promise可能成功也可能失败
      try {
        // 成功
        await this.$store.dispatch('addOrUpdateShopCart', {skuId: this.skuId, skuNum: this.skuNum})
        // 第二步：根据请求添加购物车返回信息决定是否跳转
        // 跳转添加购物车成功页面
        // 往添加购物车成功页面跳转的时候需要带两个东西，skuNum和商品详情信息
        // skuNum是个简单数据，直接通过路由传参
        // 商品详情信息是一个复杂的数据，我们最好不要通过路由传参，因此我们采用存储方案。sessionStorage
        alert('添加购物车成功，准备去往成功页面')

        sessionStorage.setItem('SKUINFO_KEY',JSON.stringify(this.skyInfo))
  ...
  computed: {
    ...mapGetters(['categoryView', 'skuInfo', 'spuSalueAttrList']),
    imgList(){
      // 确保不是undefind
